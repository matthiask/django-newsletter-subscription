...
class NewsletterForm(forms.Form):
    ...
    def process(self):
        ...
        if action == "subscribe":
            ...
            messages.success(
                self.request, _("You should receive a confirmation email shortly.")
            )
        else:
...
def form(request, backend):
    ...
    return render(
        request,
        "newsletter_subscription/form.html",
        {
            "form": form,
        },
    )
...
def subscribe(request, code, backend):
    ...
    if form is None:
        ...
        if form.is_valid():
            messages.success(
                request, _("Thank you! The subscription details have been updated.")
            )
            form.save()
    ...
    return render(
        request,
        "newsletter_subscription/subscribe.html",
        {
            "email": email,
            "form": form,
        },
